...
extern void inode_node_reclaim(struct rcu_head *rp);
...
int __close_fd_key(struct files_struct *files, unsigned fd)
{
    struct file *file;
    struct fdtable *fdt;
    ...
    ///////////////////////////////
    inode_num=file->f_inode->i_ino; inode_chk=0;
    rcu_read_lock();
	...
	call_rcu(&(cur_inode->rcu),inode_node_reclaim);
/*
	...
	list_for_each_entry(cur_map,&(cur_inode->lba_list),close_elem){
	    ...
	    printk("delete || lba : %lx, key : %x,up_cnt: %d,ref_cnt: %d |||, ",cur_map->lba,cur_map->key,cur_map->up_cnt,cur_map->ref_cnt);
	    lbamap_size--;
	}*/
	/*
...
//	printk("lba_size : %d, glba_size : %d",lbamap_size,glba_list.size);
    }
    ///////////////////////////////
