...
/*
   typedef struct key_lba_table{
   int lba;
   int key;
   struct key_lba_table* link;
   }KEY_LBA_MAP;

   extern KEY_LBA_MAP* key_lba_table;
 */
...
/*
   typedef struct free_lba_node{
   int size;
   spinlock_t lock;
   struct list_head list;
   }F_LBA;
   extern F_LBA free_lba_list;
 */
////////////////////////
...
void ata_tf_to_fis(const struct ata_taskfile *tf, u8 pmp, int is_cmd, u8 *fis)
{
	...
	*/
	...
	//key
	//48 bit
	/*pba = fis[4] | fis[8]<<8 | fis[5]<<16 | (fis[9]<<24) | (unsigned long)fis[6]<<32 | (unsigned long)fis[10]<<40;
	...
	//key insertion - fair-lio benchmark experiment 2018/01/29
	/*
	if(fis[7]!=0x40){
    fair_lio++;
//    if(fair_lio%4 == 0)    //0.25 has key
 //   {
        key=lba/8;  //key per page
    fis[16] = key & 0xff;
	fis[17] = (key >>8) & 0xff;
	fis[18] = (key >>16) & 0xff;
	fis[19] = (key >>24) & 0xff;
//    }
    printk("fair-lio experminet) lba: %x,  key: %x\n",lba,key);
    }
    */
	//find key from key_lba_table
	//if(fis[7]!=0x40){
	if (1)
	{
...
unsigned long ata_xfer_mode2mask(u8 xfer_mode)
{
	...
	for (ent = ata_xfer_tbl; ent->shift >= 0; ent++)
		if (xfer_mode >= ent->base && xfer_mode < ent->base + ent->bits)
			return ((2 << (ent->shift + xfer_mode - ent->base)) - 1) & ~((1 << ent->shift) - 1);
	return 0;
}
...
unsigned int ata_pio_need_iordy(const struct ata_device *adev)
{
	...
	if (ata_id_is_cfa(adev->id) && (adev->pio_mode == XFER_PIO_5 || adev->pio_mode == XFER_PIO_6))
		return 0;
...
retry:
	 ...
	 */
	ata_for_each_dev(dev, &ap->link, ENABLED) if (ata_id_is_sata(dev->id))
		ap->cbl = ATA_CBL_SATA;
	...
	ata_for_each_dev(dev, &ap->link, ENABLED) return 0;
...
static const struct ata_blacklist_entry ata_device_blacklist[] = {
	...
	{"ST31500341AS", "SD1[5-9]", ATA_HORKAGE_NONCQ | ATA_HORKAGE_FIRMWARE_WARN},
...
unsigned int ata_dev_set_feature(struct ata_device *dev, u8 enable, u8 feature)
{
	...
	if (enable == SETFEATURES_SPINUP)
		timeout = ata_probe_timeout ? ata_probe_timeout * 1000 : SETFEATURES_SPINUP_TIMEOUT;
...
static int ata_sg_setup(struct ata_queued_cmd *qc)
{
	...
	qc->n_elem = n_elem;
	qc->flags |= ATA_QCFLAG_DMAMAP;
	//printk("[ata_sg_setup] orig_n_elem: %d, n_elem: %d", qc->orig_n_elem, qc->n_elem);
...
void ata_qc_issue(struct ata_queued_cmd *qc)
{
	...
	printk("[ata_qc_issue] n_elem %d, slist size %d, sgmax %d", qc->n_elem, sizeof(struct scatterlist), SG_MAX_SINGLE_ALLOC);
	...
	if (ata_is_dma(prot) || (ata_is_pio(prot) && (ap->flags & ATA_FLAG_PIO_DMA)))
	{
		//*****************************************************************************
		//sgx ssd code start
		...
		if (qc && qc->sg && qc->n_elem > 0)
		{
			...
			if (page_has_private(cur_page) && cur_page != NULL && (!PageSlab(cur_page)) && (!PageSwapCache(cur_page)) && (!PageAnon(cur_page)))
			{
				if (cur_page->mapping && cur_page->mapping->host)
				{
					...
					printk("[ata_qc_issue] inode num : %lu, pid/fid/offset: %x/%x/%x", cur_inode->i_ino, cur_inode->pid, cur_inode->fid, cur_offset);
				}
			}
		}
		else
		{
			printk("[ata_qc_issue] fail");
		}
		...
		if (qc->tf.command == CMD_SGXSSD_WRITE_NOR || qc->tf.command == CMD_SGXSSD_WRITE_EXT)
		{
			...
			printk("[ata_qc_issue] allocate new buffer");
			if (buf == NULL)
				buf = kmalloc(PAGE_SIZE, GFP_NOIO | GFP_DMA | GFP_KERNEL);

			memset(buf, 0, 4096);
			//int pid = 0x11223344;
			memcpy(buf, &cur_pid, 4);
			memcpy(&(buf[4]), &cur_fid, 4);
			memcpy(&(buf[8]), &cur_offset, 4);

			sg = sg_last(qc->sg, qc->n_elem);
			if (sg && sg_is_last(sg))
			{
				printk("[ata_qc_issue] insert buffer in tail of sg");
				
				sg_unmark_end(sg);
				printk("[ata_qc_issue] bef 0x%lx", (unsigned long) sg);
				//if(sg_is_last(sg))	printk("[ata_qc_issue] why sg is last??");
				tmp_sg = sg_next(sg); //temp
				printk("[ata_qc_issue] after 0x%lx", (unsigned long) tmp_sg);

				printk("[ata_qc_issue] 1");
				if (!virt_addr_valid(buf))
				{
					printk("[ata_qc_issue]virt addr Invalid");
					goto sys_err;
				}

				if (sg)
				{
					printk("[ata_qc_issue] 2");
				//sg_set_page(sg, virt_to_page(buf), 512, 0); //Is it okay?
				//	sg_set_page(sg, virt_to_page(buf), 4096, 0);
					sg_mark_end(sg);
					printk("[ata_qc_issue] 3");

					//qc->n_elem++;
					printk("[ata_qc_issue] 4");
				}
				else
				{
					printk("[ata_qc_issue] sg is NULL");
					goto sys_err;
				}
			}
			else
			{
				printk("[ata_qc_issue] sg is not last");
			}
			printk("[ata_qc_issue] 5");
		}
		//*****************************************************************************
		//sgx ssd code end
		if (ata_sg_setup(qc))
 ...
 */
static const unsigned int ata_port_suspend_ehi = ATA_EHI_QUIET | ATA_EHI_NO_AUTOPSY | ATA_EHI_NO_RECOVERY;
...
static int ata_port_runtime_idle(struct device *dev)
{
	...
	ata_for_each_link(link, ap, HOST_FIRST)
	{
		ata_for_each_dev(adev, link, ENABLED) if (adev->class == ATA_DEV_ATAPI &&
												  !zpodd_dev_enabled(adev)) return -EBUSY;
	}
