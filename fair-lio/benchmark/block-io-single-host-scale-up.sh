...
#########################################################################################
# ORNL OLCF-3 single host block level I/O scale up benchmark scipt                      #
#                                                                                       #
#########################################################################################
...
target_host="tick-oss3"

# List devices to be tested on the host below
...
out_raw_dir="block_io_single_host_scale_up_raw_${timestamp}"

out_summary_file="block_io_single_host_scale_up_${timestamp}.summary"

out_csv_file="block_io_single_host_scale_up_${timestamp}.csv"

tmp_file="temp_out_single_host_scale_up_${timestamp}.tmp"

echo -e "******************************************************"
echo -e "* ORNL OLCF Block-I/O Single Host Scale Up Benchmark *"
echo -e "******************************************************"
...
if [ "$len" -ne "0" -a "$((2**steps))" -ne "$len" ]; then
        let "steps+= 2"
elif [ "$len" -ne "0" -a "$((2**steps))" -eq "$len" ]; then
        let "steps+= 1"
fi

total_test_num=0
total_test_num=$(( $steps * ${#io_mode_arr[@]} * ${#qs_arr[@]} * ${#bs_arr[@]} * ${ITERS} ))

new_devs=""

for ((j=1; j<=$steps; j++ )); do
        index=$((2**$(($j-1))))
        new_devs=(${DEV[@]:0:$index})
	num_drvs=${#new_devs[@]}
		...
		for qs in ${qs_arr[@]}; do
			for bs in ${bs_arr[@]}; do
				for ((i = 0; i < ITERS; i++)); do
					rand=$RANDOM
					let "f_seed = $i + ${rand}"
					echo $rand $io $bs $qs $i $f_seed $num_drvs ${new_devs[@]} 
				done
			done 
		done		
	done
