...
int main(int argc, char **argv)
{
	...
	for (;;) {
		...
		case 'w':	//write
			...
			printf("write option: write_pct: %f, optarg = %s\n", write_pct, optarg);
			break;
		case 'l':
			loop_at_end = 1;
			break;
	...
	printf("max_req : %d, arena_size: %ld\n", max_req, arena_size);
...
//	end.tv_nsec = now.tv_nsec;
	...
	while(1)
	{
		...
		for (i = 0; i < count && !end_of_device; i++) {
			...
			if((unsigned long)dev->offset >= (unsigned long)SSD_CAPACITY)
			{
				end_flag = 1;
				//break;
			}
