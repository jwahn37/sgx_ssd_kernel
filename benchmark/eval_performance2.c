...
*/	
	...
	//	printf("before set name\n");
	   ...
	   //첫번째 쓰기는 파일을 전부 써야함.
		...
		//이후 덮어쓰기는 패턴에 따라 sequential, random하게 수행
		//overwrite 1 
		
		write_file(fd, file_size, buf, file_pattern_flag);
		//write_file(fd, file_size, buf, file_pattern_flag);
		//write_file(fd, file_size, buf, file_pattern_flag);
		/*	삭제 예정
	   //file size만큼
...
int write_file(int fd, int file_size, char buf[BUF_SIZE], char file_pattern_flag)
{
	...
	else if(file_pattern_flag=='R')
	{
		for(i=0; i<file_size/BUF_SIZE; i++)
		{
			random_offset = rand() % (file_size-BUF_SIZE);
			lseek(fd, random_offset, SEEK_SET);
	//		printf("[DBG] random_offset: %d\n", random_offset);
			err = syscall(WRITE_KEY, fd, buf, BUF_SIZE);
		}
	}
