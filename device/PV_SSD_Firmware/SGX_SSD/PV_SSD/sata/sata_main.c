...
UINT32 num_gc=0;
UINT32 num_move_backup=0;
UINT32 num_backup=0;
UINT32 num_backup2=0;
UINT32 num_fin1=0;
UINT32 num_fin2=0;
UINT32 num_expired1;
UINT32 num_expired2;

 UINT32 num_move_backup1=0;
 UINT32 num_move_backup2=0;
UINT32 cur_time = 0;
UINT32 r_pos_bank = 0;
...
void Main(void)
{
	...
	pvssd_policy[7].pid = 7;
	pvssd_policy[7].ret_time = 1000000;
    // pvssd_policy[3].ret_time = 3*IO_CNT_PER_DAY; //RT=3days
	pvssd_policy[7].num_version = 3;
	...
	while (1)
	{
		if (event_cnt=eventq_get_count())// && !queue_isEmpty())
		{
			...
			if (!(cmd.cmd_type <= 0x4B && cmd.cmd_type >= 0x48))
				uart_printf("0x%x with lba : 0x%x, sect_count : 0x%x: Not Write or Recovery Command!", cmd.cmd_type, cmd.lba, cmd.sector_count);
...
static void PV_recovery (UINT32 const lba, UINT32 const sect_count, UINT32 const cmd_type, UINT32 const r_meta) {
	...
	offset = ftl_recovery(lba, sect_count - SECTORS_PER_PAGE, fid, r_time);
