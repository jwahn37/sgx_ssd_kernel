...
static void move_to_backup_zone(UINT32 const bank, UINT32 const vpn, UINT32 const lpn, UINT32 const old_vpn, UINT32 const written_time, UINT32 const pid, UINT32 const fid, UINT32 const offset)
{
    ...
    UINT32 backup_vpn, old_backup_vpn[MAX_CHAIN_LEN];
    ...
    if (old_vpn != NULL)
    {
        ...
        while(1)
        {
            ...
            // old_vpn_stack[i] = old_vpn_next; // old_vpn does not need anymore 'cause it's already changed나
    ...
    while (1)
    {
        ...
        if (delta_offset == MAX_PAGES_IN_DELTA - 1)
        {
            ...
            // set write_vpn for backup_page_map!
            ...
            /* 
                [WARNING]
                Chain will be broken if a backed-up block are not reclaimed in time order!
            */

            // 1. Set Delta Header and Backup Page Table
            for (j=0; j<MAX_PAGES_IN_DELTA; j++) {
                delta_header[idx].prev_vpn[j] = get_backup_vpn(delta_header[idx].lpn[j]);
                set_backup_vpn(delta_header[idx].lpn[j], backup_vpn);
            }
        ...
        else
        {
            ...
            /* Header Update */
            UINT32 offset = delta_offset[idx];
            delta_header[idx].lpn[offset] = lpn_stack[chain_len];
            delta_header[idx].WT[offset] = WT_stack[chain_len];
            delta_header[idx].lpn[offset] = pid_stack[chain_len];
            delta_header[idx].fid[offset] = fid_stack[chain_len];
            delta_header[idx].offset[offset] = offset_stack[chain_len];
            delta_offset[idx]++;

            UINT32 vpn_tmp = get_vpn(lpn_stack[chain_len]);
            set_bit_dram(RECLAIMABILITY_BMP_ADDR + vpn_tmp / 8, vpn_tmp % 8);
        }
