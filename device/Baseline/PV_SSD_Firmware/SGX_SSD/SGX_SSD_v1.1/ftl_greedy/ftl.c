...
static UINT32 pop_free_blk_pool (free_blk_pool* f_pool);
    ...
    for ( ; i >= 0; i--) 
    {
        ...
        if (delta_offset[idx] == MAX_PAGES_IN_DELTA - 1)
        {
            ...
            for (j=0; j<MAX_PAGES_IN_DELTA; j++) {
                delta_header[idx].prev_vpn[j] = get_backup_ppn(delta_header[idx].lpn[j]);
                set_backup_ppn(delta_header[idx].lpn[j], backup_ppn);
            }
...
static UINT32 assign_new_backup_write_ppn(UINT32 const hash_index)
{
    ...
    if (is_found == FALSE)
    {
        for (new_bank=0; new_bank<NUM_BANKS; new_bank++)
        {
            ...
            bank = new_bank;
            pblock = pop_free_blk_pool(&f_pool[new_bank]);
            vblock = pblock % VBLKS_PER_BANK;
            ...
            if (map_vpn != src_vpn)
            {
                ...
                else 
                {
                    ...
                    */
                    ...
                    while (1) {
                        UINT32 bn = prev_vpn / PAGES_PER_BANK;
                        UINT32 pn = prev_vpn % PAGES_PER_BANK;
...
void insert_free_blk_pool (UINT32 bank, UINT32 vblock, free_blk_pool** f) {
...
UINT32 pop_free_blk_pool (free_blk_pool* f_pool)
{
    if (is_empty(f_pool)) {
