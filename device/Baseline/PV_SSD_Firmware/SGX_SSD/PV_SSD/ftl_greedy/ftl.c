...
struct backup_blk_stack backup_table[NUM_BUCKETS];
...
static void move_to_backup_zone(UINT32 const bank, UINT32 const vpn, UINT32 const _lpn, UINT32 const _old_vpn, UINT32 const _written_time, UINT32 const _pid, UINT32 const _fid, UINT32 const _offset, UINT32 const prev_WT);
...
static void insert_free_blk_pool (UINT32 bank, UINT32 vblock, free_blk_pool* f);
static UINT32 pop_free_blk_pool (UINT32 bank, free_blk_pool* f);
static UINT32 is_full (free_blk_pool* f_pool);
static UINT32 is_empty (free_blk_pool* f_pool);
...
static void move_to_backup_zone(UINT32 const bank, UINT32 const vpn, UINT32 const _lpn, UINT32 const _old_vpn, UINT32 const _written_time, UINT32 const _pid, UINT32 const _fid, UINT32 const _offset, UINT32 const prev_WT)
{
    ...
    UINT32 backup_ppn, pblock;
    UINT32 old_vpn_temp, page_num, vblock, bank;
    UINT32 j = 0;
    UINT32 chain_len = 0;
    ...
    UINT32 lpn, old_vpn, written_time;
    UINT32 pid, fid, offset;
    UINT32 page_addr;

    // UINT32 old_vpn_stack[MAX_CHAIN_NUM];
    UINT32 lpn_stack[MAX_CHAIN_NUM];
    UINT32 WT_stack[MAX_CHAIN_NUM];
    UINT32 pid_stack[MAX_CHAIN_NUM];
    UINT32 fid_stack[MAX_CHAIN_NUM];
    UINT32 offset_stack[MAX_CHAIN_NUM];
    UINT32 time[MAX_CHAIN_NUM];
    UINT32 expired_time;
    UINT32 invalid_time;

    invalid_time = prev_WT;
    lpn = _lpn; old_vpn = _old_vpn; written_time = _written_time;
    pid = _pid; fid = _fid; offset = _offset;
    // bank = lpn % NUM_BANKS;

    UINT32 i = -1;

    // Check if the first page is expired or not
    expired_time = invalid_time + pvssd_policy[pid].ret_time;

    // if (expired_time < cur_cnt) {
    if (expired_time < 0) // [N] tmp condition
        return;   
    else
    {
        do
        {
            // Push OOB Data in the stack
            lpn_stack[++i] = lpn;
            pid_stack[i] = pid;
            fid_stack[i] = fid;
            ...
            // Before reading pages, check if it's already reclaimed
            if(test_reclaim_bitmap(bank, old_vpn))
                break;
            // page_addr = (bank * PAGES_PER_BANK) + old_vpn;
            // if (tst_bit_dram(RECLAIMABILITY_BMP_ADDR + (page_addr / 8), page_addr % 8) == 1) // if reclaimable
            //     break;

            // Read Next Page in the chain!
            vblock = old_vpn / PAGES_PER_BLK;
            page_num = old_vpn % PAGES_PER_BLK;
            ...
            mem_copy(&lpn, FTL_BUF(bank) + (sizeof(UINT32) * page_num), sizeof(UINT32));
            mem_copy(&old_vpn, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK + sizeof(UINT32) * page_num), sizeof(UINT32));
            mem_copy(&written_time, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK * 2 + sizeof(UINT32) * page_num), sizeof(UINT32));
            mem_copy(&pid, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK * 3 + sizeof(UINT32) * page_num), sizeof(UINT32));
            mem_copy(&fid, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK * 4 + sizeof(UINT32) * page_num), sizeof(UINT32));
            mem_copy(&offset, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK * 5 + sizeof(UINT32) * page_num), sizeof(UINT32));

            expired_time = invalid_time + pvssd_policy[pid].ret_time;
            // if (lpn != _lpn || pid == NULL || expired_time < cnt)
            if (lpn != _lpn || pid == NULL || expired_time < 0) // [N] tmp condition
                break;
                
        } while (old_vpn != NULL);
    }

    uart_printf("Including the first page, chain length is %d", i + 1);
    ...
    for ( ; i >= 0; i--) 
    {
        ...
        UINT8 idx = get_bucket(time[i]); // Need to have hash function

        if (delta_offset[idx] == MAX_PAGES_IN_DELTA - 1)
        {
            backup_ppn = assign_new_backup_write_ppn(idx); // 수정 요망
            pblock = backup_ppn / PAGES_PER_BLK;
            vblock = pblock % VBLKS_PER_BANK;
            page_num = backup_ppn % PAGES_PER_BLK;
            ...
            for (j=0; j<MAX_PAGES_IN_DELTA; j++) {
                ...
                set_backup_ppn(delta_header[idx].lpn[j], bank, backup_ppn % PAGES_PER_BLK);
            }
        ...
        else
        {
            ...
            /* Header Update */
            UINT32 num = delta_offset[idx];
            delta_header[idx].lpn[num] = lpn_stack[chain_len];
            delta_header[idx].WT[num] = WT_stack[chain_len];
            delta_header[idx].lpn[num] = pid_stack[chain_len];
            delta_header[idx].fid[num] = fid_stack[chain_len];
            delta_header[idx].offset[num] = offset_stack[chain_len];
            delta_offset[idx]++;
...
static UINT32 assign_new_backup_write_ppn(UINT32 const hash_index)
{
    ...
    UINT32 write_ppn = cur_backup_write_ppn[hash_index];
    UINT32 bank, write_vpn;
    UINT32 pblock, vblock, page_num;
    UINT32 new_vblock, new_bank;

    UINT32 is_found = TRUE;
    UINT32 is_first = FALSE;
    if (write_ppn == INIT_BACKUP_WRITE_PTR)
    {
        for (bank=0; bank<NUM_BANKS; bank++)
        {
            vblock = META_BLKS_PER_BANK;
        ...
        uart_printf("first backup block is %d bank %d vblock", bank, vblock);
        is_first = TRUE;
    }
    else 
    {
        bank = write_ppn / PAGES_PER_BANK;
        write_vpn = write_ppn % PAGES_PER_BANK;
        vblock = write_vpn / PAGES_PER_BLK;
        page_num = write_vpn % PAGES_PER_BLK;

        if (page_num == PAGES_PER_BLK -1)
        {
            for (new_bank=0; new_bank<NUM_BANKS; new_bank++)
            {
                new_vblock = META_BLKS_PER_BANK;
                do
                {
                    if (++new_vblock == VBLKS_PER_BANK - 1)
                    {
                        if (new_bank == NUM_BANKS -1)
                            is_found = FALSE;
    ...
    // 첫 번째 블락인가? (is_first == TRUE?)
    // Y -> 새로 할당된 놈으로 세팅
    // N -> 새로 할당된 블락인가? ()
    //      Y -> 새로 할당된 놈으로 세팅
    //      N -> 못 찾은 놈은 아닌가?
    //           Y -> Free Block을 반환
    //           N -> Incremental 한 놈으로 세팅
    ...
    if (is_found == FALSE)
    {
        ...
        {
            // Free Block으로 세팅하자!
            // -> Bank랑 vblock아니까 Free Block Pool에서 세팅 가능!
            if (is_empty(&f_pool[new_bank]))
                continue;
            
            bank = new_bank;
            pblock = pop_free_blk_pool(bank, f_pool);
            vblock = pblock % VBLKS_PER_BANK;
            write_ppn = pblock * PAGES_PER_BLK;

            set_vcount(bank, vblock, VC_BACKUP);   
            push_backup_stack(&backup_table[hash_index], (UINT16)pblock);
            break;
        }
    }
    else if ((vblock != write_vpn / PAGES_PER_BLK && is_found == TRUE) || is_first == TRUE)
    {
        // After new physical block is allocated, update write_ppn
        bank = new_bank;
        vblock = new_vblock;
        pblock = vblock * VBLKS_PER_BANK;
        write_ppn = pblock * PAGES_PER_BLK;
        set_vcount(bank, vblock, VC_BACKUP);
        push_backup_stack(&backup_table[hash_index], (UINT16)pblock);
    }
    else // Incremental Page Allocation
    {
        write_ppn++;
    }

    // Update the cur_backup_write_ppn
    cur_backup_write_ppn[hash_index] = write_ppn;
...
static void init_metadata_sram(void)
{
    ...
    for (bank = 0; bank < NUM_BANKS; bank++)
    {
        ...
        do
        {
            ...
            /* set free block */
            insert_free_blk_pool (bank, vblock, f_pool);           
        ...
        //----------------------------------------        
        set_new_backup_write_vpn(bank, INIT_BACKUP_WRITE_PTR); // 수정
        // uart_printf("bank : %d backup ptr : %x", bank, g_misc_meta[bank].cur_backup_write_vpn); // 수정
    }

    for (i=0; i<NUM_BUCKETS; i++)
        mem_set_sram(&backup_table[i], 0, sizeof(struct backup_blk_stack));
}
...
static void modified_garbage_collection(UINT32 const bank)
{
    ...
    UINT32 prev_WT, cur_vpn, prev_vpn;
            ...
            if (map_vpn != src_vpn)
            {
                ...
                else 
                {
                    ...
                    */
                    ...
                    prev_vpn = get_vpn(src_lpn);
                    uart_print("Want to find Invalidation Time");
                    while (1) {
                        ...
                        mem_copy(&cur_vpn, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK + sizeof(UINT32) * pn), sizeof(UINT32));
                        mem_copy(&prev_WT, FTL_BUF(bank) + (sizeof(UINT32) * PAGES_PER_BLK * 2 + sizeof(UINT32) * pn), sizeof(UINT32));
                    ...
                    uart_print("Got an invalidation time");

                    // move the page and all the follwing chain into backup page
                    uart_print ("*****************\nmove chain to backup zone safely!\n*****************");
                    move_to_backup_zone (bank, src_vpn, src_lpn, src_old_vpn, src_written_time, src_pid, src_fid, src_offset, prev_WT);
                }
            }
            else
            {   //valid인 경우
...
UINT32 is_full (free_blk_pool* f_pool)
{
    return (f_pool->size == MAX_FREE_BLKS);
}

UINT32 is_empty (free_blk_pool* f_pool)
{
    return (f_pool->size == 0);
}

static void push_backup_stack (struct backup_blk_stack* b, UINT16 vblock)
{
    if (b->size == MAX_BLKS_IN_INTERVAL) {
        uart_print ("****************\n Stack is Full!\n****************");
        return;
    }

    b->size++;
    b->array[b->top++] = vblock;
}

static UINT16 pop_backup_stack (struct backup_blk_stack* b)
{
    if(b->size == 0) {
        uart_print(("****************\n Stack is Empty!\n****************");
        return -1;
    ...
    b->size--;
    return b->array[--(b->top)];
}

static void check_and_reclaim_backup_zone ()
{
    UINT32 idx = get_bucket(cur_cnt);
    UINT32 i, pblock, vblock, bank;

    while(1)
    {
        pblock = (UINT32)pop_backup_stack(&backup_table[idx]);
        
        if(pblock == -1)
            break;
        
        bank = pblock / VBLKS_PER_BANK;
        vblock = pblock % VBLKS_PER_BANK;
        nand_block_erase(bank, vblock);
        set_vcount (bank, vblock, VC_FREE);
//        insert_free_blk_pool (bank, vblock, f_pool);
    }
