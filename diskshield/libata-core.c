...
#include<linux/hashtable.h>    //key_ssd
...
*/
	...
	if((fis[7]&15)==0){ //48bit
		tp=1;
//		printk("48b");
		DS_lba=fis[4] | (fis[5]<<8)| (fis[6]<<16) | ((unsigned long)fis[8]<<24) | ((unsigned long)fis[9]<<32) | ((unsigned long)fis[10]<<40);
...
//		printk("32b");
	...
	if(lba_chk){
		if(cmd==DS_CREATE_WR)
			printk("[ata_tf_to_fis] Create Request to SATA.");	
		if(cmd==DS_OPEN_WR)
			printk("[ata_tf_to_fis] Open Request to SATA.");	
		if(cmd==DS_WRITE_WR)
			printk("[ata_tf_to_fis] Write Request to SATA.. fd: %d,offset: %d, size: %d", fd, DS_lba, tf->nsect*512);	
		if(cmd==DS_READ_RD)
			printk("[ata_tf_to_fis] Read Request to SATA.. fd: %d,offset: %d, size: %d", fd, DS_lba, tf->nsect*512);
		if(cmd==DS_CLOSE_WR)
			printk("[ata_tf_to_fis] Close Request to SATA.. fd: %d", fd);	
		//else
		//	printk("[ata_tf_to_fis] Read-after-Write Request to SATA");

		//if(cmd<=0x48)
			///printk("[ata_tf_to_fis]WR) lba: %llx, cmd : %x, fd : %x\n", DS_lba, cmd, fd);
		//else
			///printk("[ata_tf_to_fis]RD) lba: %llx, cmd : %x, version(fd) : %x\n", DS_lba, cmd, fd);
		/*
		if(tp==1)
		{
			printk("48b");
			...
			printk("32b");
		*/
		//printk("[ata_tf_to_fis] key_sata: lba %lx,value %lx,call=%u\n",DS_lba,value,call);
	/*
		fis[7]-=(fis[7]&15);
		fis[4]=value&255;		value>>=8;
		fis[5]=value&255;		value>>=8;
		fis[6]=value&255;		value>>=8;
		fis[8]=value&255;		value>>=8;
		fis[9]=value&255;		value>>=8;
		fis[10]=value&255;	
		fis[16]=call&255;
	*/
		//diskshield code
		//offset을 집어넣는다.
		fis[4]=DS_lba&255;	DS_lba>>=8;
		fis[5]=DS_lba&255;	DS_lba>>=8;
		fis[6]=DS_lba&255;	DS_lba>>=8;

		fis[8]=DS_lba&255;	DS_lba>>=8;
		fis[9]=DS_lba&255;	DS_lba>>=8;
		fis[10]=DS_lba&255;	DS_lba>>=8;
